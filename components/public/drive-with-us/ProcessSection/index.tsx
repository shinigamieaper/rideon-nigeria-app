...
export default function ProcessSection({
  ...
  const steps = [
    ...
    },
    {
      ...
      desc: "Receive pre-booked reservations and long-term contract opportunities. Drive with confidence and predictability.",
      Icon: Car,
      delay: "500ms",
    },
  ];
          ...
          <ol className="relative grid grid-cols-1 gap-10 lg:grid-cols-4">
            {steps.map(({ id, title, desc, Icon, delay }, idx) => (
              <li key={id} className="relative">
                ...
                {idx < steps.length - 1 && (
                  <div
                    className="hidden lg:block absolute top-12 left-[60%] right-[-10%] h-0.5 bg-gradient-to-r from-blue-500/60 via-indigo-500/60 to-emerald-500/60"
                    aria-hidden="true"
                  />
                )}
