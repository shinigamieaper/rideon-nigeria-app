...
import { usePartnerPushNotifications } from "@/hooks/usePartnerPushNotifications";
import { cn } from "@/lib/utils";

export interface PartnerNotificationPermissionCardProps
  extends React.ComponentPropsWithoutRef<"div"> {
  compact?: boolean;
}

export default function PartnerNotificationPermissionCard({
  compact = false,
  className,
  ...rest
}: PartnerNotificationPermissionCardProps) {
  const { flags } = useFeatureFlags();
  const { status, isEnabled, enablePush, error } =
    usePartnerPushNotifications();
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-emerald-600/80 dark:text-emerald-500/80">
              You’ll receive updates about reservations and payouts
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
            Get notified when new reservation requests arrive and when payouts
            are processed.
          </p>
          <button
            onClick={() => enablePush()}
