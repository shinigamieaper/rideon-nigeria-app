...
export interface NotificationTogglesProps
  extends React.ComponentPropsWithoutRef<"div"> {}

// Matrix: single source of truth
// Only channels listed should be rendered.
const SPEC = [
  {
    key: "my_trips",
    ...
    items: [
      {
        key: "trip_confirmation",
        title: "Reservation Confirmation",
        desc: "Sent immediately after a successful reservation.",
        channels: { push: true, email: true },
      },
      {
        key: "trip_reminder",
        title: "Reservation Reminder",
        desc: "1 hour before scheduled pickup.",
        channels: { push: true, email: false },
      },
      {
        key: "driver_assigned",
        title: "Driver Assigned",
        desc: "Your driver is confirmed.",
        channels: { push: true, email: true },
      },
      {
        key: "driver_en_route",
      ...
      {
        key: "trip_completed_receipt",
        title: "Reservation Completed & Receipt",
        desc: "Receipt emailed on completion.",
...
export default function NotificationToggles({
  ...
  // use shared waitForUser() from lib/firebase to avoid flaky short timeouts
  ...
  React.useEffect(() => {
    ...
    async function run() {
      try {
        ...
        if (!res.ok)
          throw new Error(j?.error || "Failed to load notification prefs");
        if (!cancelled) setPrefs(j);
      } catch (e: any) {
        console.error(e);
        if (!cancelled)
          setError(e?.message || "Failed to load notification prefs.");
      } finally {
  ...
  async function save(updated: Prefs) {
    try {
      ...
      const j = await res.json().catch(() => ({}));
      if (!res.ok)
        throw new Error(j?.error || "Failed to save notification prefs");
      setPrefs(j);
    } catch (e: any) {
      console.error(e);
      setError(e?.message || "Failed to save notification prefs.");
    }
  }
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-400">
                    {prefs.enabled
                      ? "You'll receive updates about your reservations"
