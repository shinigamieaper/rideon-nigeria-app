...
export interface AdminNotificationPermissionCardProps
  extends React.ComponentPropsWithoutRef<"div"> {
  compact?: boolean;
}

export default function AdminNotificationPermissionCard({
  compact = false,
  className,
  ...rest
}: AdminNotificationPermissionCardProps) {
  const { flags } = useFeatureFlags();
  const { status, isEnabled, enablePush, error } = useAdminPushNotifications();
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-emerald-600/80 dark:text-emerald-500/80">
              You’ll receive operational alerts and updates
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
            Get alerted about operational issues, delivery failures, and
            important updates.
          </p>
          <button
            onClick={() => enablePush()}
