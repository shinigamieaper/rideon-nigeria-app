...
/**
 * Card component for enabling/managing push notification permissions
 * Shows different states based on notification status
 */
export default function NotificationPermissionCard({
  ...
  const { status, isEnabled, enablePush, error } = usePushNotifications();
  ...
  // Already enabled - show success state (can be hidden in compact mode)
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-emerald-600/80 dark:text-emerald-500/80">
              You'll be notified when new booking requests arrive
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
            Get notified instantly when new booking requests are assigned to
            you.
          </p>
          <button
            onClick={() => enablePush()}
