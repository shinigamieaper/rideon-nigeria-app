...
import { Briefcase, Check, MessageSquare, X } from "lucide-react";

import { auth } from "@/lib/firebase";
import StickyBanner from "@/components/ui/StickyBanner";
import type { PlacementHireRequest } from "@/types/placement";

export interface HireRequestsPanelProps
...
export default function HireRequestsPanel({
  className,
  ...rest
}: HireRequestsPanelProps) {
  const [loading, setLoading] = React.useState(true);
  const [error, setError] = React.useState<string | null>(null);
  const [requests, setRequests] = React.useState<PlacementHireRequest[]>([]);
  ...
  const fetchRequests = React.useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch("/api/driver/placement/hire-requests", {
      ...
      if (!res.ok) {
        ...
        throw new Error(j?.error || "Failed to fetch hire requests");
      }
    ...
    async (requestId: string, action: "accept" | "decline") => {
      try {
        ...
        const res = await fetch(
          `/api/driver/placement/hire-requests/${requestId}`,
          {
            method: "POST",
            headers: {
      ...
      {loading ? (
        ...
        <div className="text-center py-10">
          ...
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">
            Hire requests appear after a client is ready to proceed.
          </p>
        </div>
      ) : (
        <div className="space-y-3">
