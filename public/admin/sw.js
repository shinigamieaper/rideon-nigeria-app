...
self.addEventListener('push', (event) => {
  console.log('[SW:Admin] Push event received:', event);

  let data = { title: 'RideOn Admin', body: 'You have a new notification' };

  if (event.data) {
    try {
      data = event.data.json();
    } catch (e) {
      data = { title: 'RideOn Admin', body: event.data.text() };
    }
  }

  const title = data.notification?.title || data.title || 'RideOn Admin';
  const options = {
    ...
    renotify: true,
    data: {
      url: data.fcmOptions?.link || data.data?.clickAction || '/admin/notifications',
      ...data.data,
    },
    actions: [
...
self.addEventListener('notificationclick', (event) => {
  console.log('[SW:Admin] Notification click:', event.action, event.notification.data);
