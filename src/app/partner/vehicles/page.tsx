...
import { AlertCircle, Car, ChevronRight, Loader2 } from "lucide-react";
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-600 dark:text-slate-400">
            Submit vehicles for approval, track review status, and respond to
            changes requests.
          </p>
        </div>
      ...
      <div className="rounded-2xl bg-white/50 dark:bg-slate-900/50 backdrop-blur-lg border border-slate-200/80 dark:border-slate-800/60 shadow-lg overflow-hidden">
        {loading ? (
          ...
          <div className="divide-y divide-slate-200/50 dark:divide-slate-800/60">
            {rows.map((r) => {
              const title = `${r.make} ${r.model}`.trim() || "Vehicle";
              const subtitle = `${r.category || "—"}${r.seats ? ` • ${r.seats} seats` : ""}`;
              const canEdit =
              ...
              return (
                <Link
                  key={r.id}
                  href={`/partner/vehicles/submissions/${encodeURIComponent(r.id)}`}
                    ...
                    <div className="flex items-center gap-3 flex-shrink-0">
                      <div className="text-right">
                        <p className="text-sm font-semibold text-slate-900 dark:text-white">
                          {formatNgn(r.partnerBaseDayRateNgn)}
                        </p>
