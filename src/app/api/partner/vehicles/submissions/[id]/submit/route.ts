  ...
  try {
    ...
    try {
      ...
      adminsSnap.docs.forEach((doc) => {
        ...
        if (
          !["super_admin", "admin", "ops_admin", "product_admin"].includes(role)
        )
          return;
      ...
      const adminLinkPath = "/admin/vehicle-submissions";
      const title = "New vehicle submission";
      const body = "A partner submitted a vehicle for review.";
      ...
      await Promise.allSettled(
        adminUids.map((adminUid) =>
          sendNotificationToAdminUser(adminUid, {
            ...
            body,
            data: {
              type: "partner_vehicle_submission_pending",
              submissionId: docId,
              partnerId: ctx.partnerId,
            },
      ...
      try {
        ...
        if (resend && from && adminEmails.length > 0) {
          ...
          const subject = "RideOn Admin: New vehicle submission";
          const text = [
            "A partner submitted a vehicle for review.",
            "",
            "Review:",
            link,
          ].join("\n");
          const html = `
            <p>A partner submitted a vehicle for review.</p>
        ...
        console.error(
          "[partner/vehicles/submissions/[id]/submit] Failed sending admin emails:",
          e,
        );
      }
    } catch (e) {
      console.error(
        "[partner/vehicles/submissions/[id]/submit] Failed notifying admins:",
        e,
      );
    }
