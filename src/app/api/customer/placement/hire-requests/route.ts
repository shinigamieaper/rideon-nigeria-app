...
function requestStatusRank(status: string): number {
  ...
  if (s === "accepted" || s === "admin_approved") return 3;
...
export async function GET(req: Request) {
  try {
    ...
    console.error("[GET /api/customer/placement/hire-requests] Error:", error);
...
export async function POST(req: Request) {
  try {
    ...
    if (!hasAccess) {
      return NextResponse.json(
        { error: "You need active access to express hiring interest." },
        { status: 403 },
      );
    }
    ...
    try {
      ...
      await adminDb.runTransaction(async (tx) => {
        ...
        const ctx =
          existing?.context && typeof existing.context === "object"
            ...
            : {};
        const prev =
          typeof ctx?.placementHireStatus === "string"
            ? String(ctx.placementHireStatus)
            : "";
        if (!prev) {
          tx.update(convRef, {
            "context.placementHireStatus": "requested",
      ...
      console.warn(
        "[POST /api/customer/placement/hire-requests] Failed to set placementHireStatus on conversation",
        e,
      );
    }

    const requestRef = adminDb.collection("placement_hire_requests").doc();
    ...
    try {
      await adminDb
        ...
        .add({
          type: "placement_hire_request_new",
          title: "New hire request",
          message: `${customerName} expressed hiring interest.`,
          unread: true,
          createdAt: FieldValue.serverTimestamp(),
        });
    } catch (e) {
      console.warn(
        "[POST /api/customer/placement/hire-requests] failed to notify driver",
        e,
      );
    }
    ...
    console.error("[POST /api/customer/placement/hire-requests] Error:", error);
    return NextResponse.json(
