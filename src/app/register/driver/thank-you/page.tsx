  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        // If the role claim was just set, token may be stale.
        if (res.status === 403) {
      ...
      >
        ...
        <div className="mt-6">
          {loading ? (
            ...
            <p className="text-sm text-emerald-700 dark:text-emerald-400 font-medium">
              Approved! You can enter the Driver app now.
            </p>
          ) : status === "rejected" ? (
        ...
        <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-3">
          {status === "approved" ? (
            ...
            <button
              onClick={() => {
                /* manual refresh */ const u = auth.currentUser;
                if (!u) return;
                u.getIdToken()
                  .then((t) =>
                    fetch("/api/drivers/me", {
