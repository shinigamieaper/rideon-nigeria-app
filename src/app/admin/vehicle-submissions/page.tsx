...
import { Loader2, AlertCircle, Check, X, ClipboardList } from "lucide-react";
import {
  ActionModal,
  Select,
  SelectContent,
...
export default function AdminVehicleSubmissionsPage() {
  ...
  const fetchSubmissions = async (token: string) => {
    ...
    const res = await fetch(`/api/admin/vehicle-submissions?${params}`, {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        setError(message || "Failed to load submissions");
      } finally {
        setLoading(false);
      }
    });
  ...
  const handleAction = async (
    ...
    try {
      ...
      const res = await fetch("/api/admin/vehicle-submissions", {
        method: "PATCH",
        headers: {
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-600 dark:text-slate-400">
                Review partner-submitted vehicles and approve into catalog
              </p>
            </div>
          </div>
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-right">
                          <span className="font-semibold text-slate-900 dark:text-white text-sm">
                            {formatNgn(s.partnerBaseDayRateNgn)}
                          </span>
                        </td>
